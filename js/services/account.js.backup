 ...
 * @returns {Promise<object>} Account data with accountId, address, privHex, pubXHex, pubYHex
 */
 ...
 * @param {string} privHex - Private key in hex format
 * @returns {Promise<object>} Account data
 */
export async function importLocallyFromPrivHex(privHex) {
...
export async function addNewSubWallet() {
  ...
  try {
    ...
    if (typeof window.renderWallet === 'function') {
      try { window.renderWallet(); } catch { }
    }
    if (typeof window.updateWalletBrief === 'function') {
      try {
        window.updateWalletBrief();
        requestAnimationFrame(() => window.updateWalletBrief());
        setTimeout(() => window.updateWalletBrief(), 0);
      } catch { }
    }
    ...
    showUnifiedSuccess(t('modal.walletAddSuccess'), t('modal.walletAddSuccessDesc'), () => {
      ...
      if (typeof window.updateWalletBrief === 'function') {
        try { window.updateWalletBrief(); } catch { }
      }
    });
 ...
 * @param {boolean} showToastNotification - Whether to show toast notification
 * @returns {Promise<object|null>} Created account data or null on failure
 */
export async function handleCreate(showToastNotification = true) {
