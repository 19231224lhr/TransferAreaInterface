...
**核心模块**:
1. **加密核心** (`js/utils/keyEncryption.ts` - 400+ 行)
   - AES-256-GCM 加密算法
   - PBKDF2 密钥推导（100,000 次迭代）
   - 完整的加密/解密/验证功能
   - 密码修改和迁移支持

2. **UI 集成** (`js/utils/keyEncryptionUI.ts` - 300+ 行)
   - 密码输入弹窗（支持确认模式）
   - 密码强度指示器（弱/中/强）
   - 密码匹配指示器（实时反馈）
   - 自动迁移旧版明文私钥
...
3. **页面集成**:
   - ✅ **新用户注册**: `newUser.js` → `setPassword.js` 流程
     - 生成密钥对 → 设置密码 → 加密存储
   - ✅ **用户登录**: `login.js` 
     - 导入私钥 → 设置密码 → 加密存储
   - ✅ **锁屏功能**: `screenLock.ts`
...
```
新用户注册:
生成密钥对 → 显示密钥信息 → 设置密码（≥6位）→ 确认密码 → 
AES-256-GCM 加密 → 保存加密数据 → 清除明文私钥 → 进入系统
...
锁屏/解锁:
10分钟无操作 → 自动锁定 → 输入密码 → 验证密码 → 解锁
```

**安全特性验证**:
- ✅ 加密算法: AES-256-GCM（行业标准）
- ✅ 密钥推导: PBKDF2，100,000 次迭代
- ✅ 随机性: 每次加密使用新的 salt（16字节）和 IV（12字节）
- ✅ 存储隔离: `encryptedPrivateKey_{accountId}` 独立存储
- ✅ 明文清除: 用户对象不再包含 `privHex` 字段
- ✅ 密码验证: 锁屏解锁使用加密密钥验证
- ✅ 向后兼容: 支持检测和迁移旧版明文私钥
...
**结论**: 私钥明文存储安全风险已完全解决，所有用户私钥现在都采用 AES-256-GCM 加密存储，即使 localStorage 泄露也无法直接窃取私钥。系统已在新用户注册、用户登录、锁屏解锁等关键流程中成功集成加密功能。
