# TransferAreaInterface 文档（交接版）

这套文档面向**开发者**，目标是让开发者在最短时间内：
1) 完全准确理解「快速转账」在前端的功能与页面构造（担保组织 / 担保委员会 / TXCer / 胶囊地址等如何在 UI 里体现）；  
2) 快速理解前端代码的核心构造（路由、模板、状态、业务服务、签名/序列化等关键模块）；  
3) 对后续开发有清晰路线（改哪里、怎么加功能、风险点是什么）。

> 说明：`docs/site/` 是产品内置「文档页」使用的双语 Markdown（`/#/docs` 会加载它们）。  
> 本交接文档是给开发者看的，放在 `docs/*.md`（不参与运行时加载）。
